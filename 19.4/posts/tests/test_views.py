...
class PostsListPaginationTest(TestCase):
    def setUp(self):
        self.client = Client()
        author = Author.objects.create(email="pagination@example.com")
        for i in range(7):
            Post.objects.create(title=f"Post {i}", content="Content", author=author)

    def test_first_page_has_5_posts(self):
        response = self.client.get("/posts/")
        self.assertEqual(response.status_code, 200)
